...
$result = mysqli_query($conn, "SELECT id,fname,lname,email,grade,address FROM student");
...
$result = mysqli_query($conn, "SELECT id,fname,lname,phone,email,address FROM instructor");
...
$result = mysqli_query($conn, "SELECT stuid, feedback, date FROM feedback");
...
if (isset($_POST['Dstudent'])) {
    $res = mysqli_query($conn, "DELETE FROM student WHERE id='$studid'");
}

if (isset($_POST['DiD'])) {
    $res = mysqli_query($conn, "DELETE FROM instructor WHERE id='$instid'");
}
...
if (isset($_POST['Astudent'])) {
    ...
    $stmt->bind_param("ssssisi", $fname, $lname, $email, $hashed_password, $grade, $address, $manid);
...
if (isset($_POST['AFD'])) {
    ...
    // Prepare SQL statement with placeholders
    $stmt = $conn->prepare("INSERT INTO instructor (fname, lname, special, email, password, phone, manid, address) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    
    // Bind the variables to the placeholders in the SQL statement
    $stmt->bind_param("ssssisss", $fname, $lname, $ispi, $email, $password, $phone, $manid, $address);
